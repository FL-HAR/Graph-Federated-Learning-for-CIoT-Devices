...
def main():
    ...
    if args.paper_graph=='Y':
      ...
      Gph = Gf.graph_in_the_paper()
      plt.pause(1)
      
    else:
      N_clients = args.N_clients
      Gph = Gf.graph_ex(N_clients)
    ...
    data_train_new, label_train_new, data_test_new, label_test_new = create_extreme_hetero_data(dataset_train,dataset_test,N_clients,10,24,args.label_hetro)
    ...
    model_number = 4+1 # only four graph filters for now
    ...
    global_var_flat = Gf.flatten(global_var)
    ...
    for round in range(0,rounds):
      ...
      client_list = sorted(client_list, key = lambda x: random.random() )
      # if round % 5 ==0:
      #     model_FedAvg.save_weights("/content/drive/MyDrive/Fed_learning/models_materials/model_FedAvg_h_h_v1_n_class=4.h5")
      #     for i in range(1,model_number):
      #       sio.savemat("/content/drive/MyDrive/Fed_learning/models_materials/G"+str(i)+"_h_h_v1_n_class=4",{"G":G_list[i]})
      #     sio.savemat("/content/drive/MyDrive/Fed_learning/models_materials/accuracy_Fedfilt_local_h_h_v1_n_class=4",{"acc_fedfilt":accuracy_Fedfilt_local})
      #     sio.savemat("/content/drive/MyDrive/Fed_learning/models_materials/accuracy_Fedfilt_general_h_h_v1_n_class=4",{"acc_fedfilt":accuracy_Fedfilt_general})
      #     sio.savemat("/content/drive/MyDrive/Fed_learning/models_materials/accuracy_FedAvg_local_h_h_v1_n_class=4",{"acc_fedavg":accuracy_FedAvg_local})
      #     sio.savemat("/content/drive/MyDrive/Fed_learning/models_materials/accuracy_FedAvg_general_h_h_v1_n_class=4",{"acc_fedavg":accuracy_FedAvg_general})
      for client in client_list:
        print("Client ID ",client)

        ## shuffling data manually
        client_data_size = data_train_new[client].shape[0]
        # d_indx = range(client_data_size)
        # d_indx = sorted(d_indx, key = lambda x: random.random() )
        # dataset_train[client][1] = dataset_train[client][1][d_indx]
        # dataset_train[client][0] = dataset_train[client][0][d_indx]
        ##
